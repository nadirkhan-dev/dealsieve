...
> **Live preview:** https://nadirkhan-dev.github.io/dealsieve/ (static, pre-computed demo results)  ·  **Video walkthrough:** _add link here_
>
> The full app, with live crawling and scoring, runs locally with `docker compose up --build`.
...
**What is deployed today.** The link at the top is a **static preview on GitHub Pages**: a
pre-computed run of the demo dataset, published because the free tiers that host a
container (Render, Hugging Face Spaces) now need a payment card. It is the real React
app reading a JSON snapshot instead of an API, so the dashboard, filters, score
breakdowns, evidence, outreach briefs and CSV export all work, and a banner says so on
every page. Crawling, scoring, CSV import and saving the buy box need the backend and are
disabled there with an inline note.

- **Preview:** `scripts/build_snapshot.py` runs the FastAPI app in demo mode against a throwaway SQLite database and writes every API response to `frontend/public/snapshot.json`. `.github/workflows/pages.yml` rebuilds that snapshot and publishes `frontend/dist` on every push to `main`. No server, no database, no API keys.
- **Full app:** one **multi-stage Docker image** serves the API and the built React app together, so there is a single deploy and no CORS setup. `docker compose up --build` runs it with Postgres locally.
- **Production target:** the same image on **Google Cloud Run** (Artifact Registry, Postgres on Neon or Supabase). The container already reads `$PORT`, so it needs no changes; this is the intended home once hosting is paid for, and is not currently running.
- **Deploy process:** push to `main` → GitHub Actions runs the tests and the frontend build, and publishes the Pages preview → `gcloud run deploy --source .` rolls out the container when a paid target is available.
...
- **Data persistence:** if `DATABASE_URL` is unset the app falls back to SQLite *inside the container*. On an ephemeral filesystem (Cloud Run, or Render's free tier) leads reset whenever the instance restarts or spins down. That is fine for a demo — with `SEED_DEMO=true` it re-imports and re-scores the sample dataset on the next boot — but set `DATABASE_URL` to a Neon or Supabase Postgres URL for anything you need to keep.
...
│                          api.js + staticApi.js (offline data layer for the preview)
├── frontend/public/       snapshot.json, generated by scripts/build_snapshot.py
├── data/                  sample CSV, recorded demo websites, real-world test list
├── scripts/               demo data generator, build_snapshot.py (Pages preview),
│                          reset_demo.sh / .ps1 (video helper)
├── Dockerfile, docker-compose.yml, render.yaml
└── .github/workflows/     ci.yml (tests) + pages.yml (static preview deploy)
```
